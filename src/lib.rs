    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            JSONFileError::InvalidJSON(error, file_path) => {
                write!(
                    f,
                    "{}[Invalid JSON Error]{} {}. Could not read/write: {} {} {}",
                    color::Fg(color::Red),
                    style::Reset,
                    error,
                    color::Fg(color::Cyan),
...
fn validate_files(
    ...
    files_validator_progress_bar.set_style(
        ProgressStyle::default_bar()
            .template("{spinner:.green} [{elapsed_precise}] [{bar:.green}]▏{pos}/{len}")
            .progress_chars("█▇▆▅▄▃▂▁  "),
    );
...
pub fn json_combine(file_paths: Vec<String>, mut writer: impl Write) {
    ...
    files_write_progress_bar.set_style(
        ProgressStyle::default_bar()
            .template("{spinner:.green} [{elapsed_precise}] [{bar:.green}]▏{pos}/{len}") // green does not work, it is always white
            .progress_chars("█▇▆▅▄▃▂▁  "),
    );
